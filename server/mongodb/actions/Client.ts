...
export async function changeStatus(status: Status): Promise<void> {
    ...
    await dbConnect()
    const client = await ClientSchema.findOneAndUpdate({ accountId: accountId }, {
...
export async function addDocument(document: File, accountId: Client['accountId']): Promise<void> {
    await dbConnect()
    const client = await ClientSchema.findOneAndUpdate({ accountId }, {
        $push: {
            documents: document
        }
    })
...
export async function getUtilityApplicants(utilityCompany: Client['utilityCompany']): Promise<Client[]> {
    ...
    const utility = await CompanySchema.findOne({ accountId: utilityCompany }) as Company
